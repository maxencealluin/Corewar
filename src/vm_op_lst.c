...
/*   Updated: 2019/05/01 11:23:21 by fnussbau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		op_load(t_vm *vm, t_process *p)
{
	...
	{
		res = read_arena(vm, (i + 1) % IDX_MOD + MEM_SIZE, DIR_SIZE);
		assign_reg(p, r, res);
	}
	else if (vm->enc_byte[0] == IND_SIZE)
	{
		res = read_arena(vm, (i + 1) % IDX_MOD + MEM_SIZE, IND_SIZE);
